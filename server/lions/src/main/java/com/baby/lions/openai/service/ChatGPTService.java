...
public class ChatGPTService {
	...
	private static final String BASE_PROMPT = "사용자의 입력을 바탕으로 4개의 일정을 생성하고, 각 일정의 이유를 엄청 간략하게 설명해주세요. " +
			"답은 앞에 ~해서는 빼고 딱 잘라서 ~하기 좋은 날이에요, ~하시는 건 어떤가요? 이런식으로 공손하게 형식은 다음과 같습니다: [\n" +
	...
	public String createSchedules(String prompt) throws JsonProcessingException {
		resetRepositories();
		String input = extractPrompt(prompt);
		String basePrompt = BASE_PROMPT + input;

		ChatGPTResponse response = getChatGPTResponse(basePrompt);
		String content = extractContent(response);

		chatRecordService.saveChatRecord(input, content);

		List<Schedule> schedules = parseSchedules(content);
	...
	private String extractPrompt(String prompt) throws JsonProcessingException {
		try {
			Map<String, String> promptMap = objectMapper.readValue(prompt, Map.class);
			String input = promptMap.get("prompt");
			if (input == null) {
	...
	private ChatGPTResponse getChatGPTResponse(String basePrompt) {
	...
	private String extractContent(ChatGPTResponse response) {
		return response.getChoices().get(0).getMessage().getContent();
	}

	private List<Schedule> parseSchedules(String content) throws JsonProcessingException {
		try {
			return scheduleService.parseSchedules(content);
		} catch (JsonProcessingException e) {
	...
	private void saveSchedules(List<Schedule> schedules) throws JsonProcessingException {
